...
def load_novia_data(data_dir, max_samples=None):
    ...
    width, dielectric_thickness = 0.041, 0.635  # 0.055mm 线宽，0.1mm 介质厚度（固定值）
    ...
    for pcb_id in pcb_ids:
        ...
        num_nodes = len(xs)
        dist_from_start: NDArray[np.float32] = np.sqrt((xs - xs[0])**2 + (ys - ys[0])**2).astype(np.float32)
        node_features = np.stack((xs, ys, dist_from_start), axis=1).astype(np.float32)
