    ...
    for file_path in npz_files[:60]: # 读取60个样本进行统计分析
        try:
            with np.load(file_path) as data:
                ...
                # 2. 计算 S 参数的模值 (Magnitude)
                magnitude = np.sqrt(real**2 + imag**2)
                
                # 3. 严格确保频点轴 (2500) 位于矩阵的最后一个维度
